...
- [ ] **User Plan Correspondence**                                               
  - [ ] Create a centralized feature access system:                              
    - [ ] Create `lib/config/plan-features.ts` to define feature access by plan  
type:                                                                            
      ```typescript                                                              
      ...
      export const planFeatures = {                                              
        ...
        premium: {                                                               
          ...
          maxAPICalls: 1000,                                                     
          features: ['advanced_ai', 'advanced_storage', 'advanced_analytics',    
'priority_support']                                                              
        },                                                                       
        enterprise: {                                                            
          maxStorage: 500, // MB                                                 
          ...
          maxAPICalls: 10000,                                                    
          features: ['premium_ai', 'unlimited_storage', 'advanced_analytics',    
'priority_support', 'team_collaboration']                                        
    ...
    - [ ] Create `lib/config/plan-access.ts` with helper functions:              
      ...
      export function getResourceLimit(planType: string, resourceName: string):  
number {                                                                         
        const plan = planType && planFeatures[planType] ? planType : 'free';     
        const limitKey = `max${resourceName.charAt(0).toUpperCase() +            
resourceName.slice(1)}`;                                                         
      ...
      export function hasFeatureAccess(planType: string, featureName: string):   
boolean {                                                                        
        const plan = planType && planFeatures[planType] ? planType : 'free';     
    ...
    - [ ] Update `lib/config/useConfig.ts` to expose plan feature helpers:       
      ```typescript                                                              
      import { getResourceLimit, hasFeatureAccess, getPlanFeatures } from        
'./plan-access';                                                                 
        ...
        <div className="space-y-6">                                              
          <div className="grid gap-6 md:grid-cols-1 lg:grid-cols-2">             
            {hasFeatureAccess(currentPlan?.planType || 'free', 'basic_ai') ? (   
              <AIAssistant                                                       
maxInteractions={getResourceLimit(currentPlan?.planType || 'free',               
'AIInteractions')} />                                                            
            ) : (                                                                
        ...
        return (                                                                 
          <Card>                                                                 
            <CardHeader>                                                         
              ...
              <CardDescription>                                                  
                {description || `Access to ${feature} requires a premium         
subscription.`}                                                                  
              </CardDescription>                                                 
            </CardHeader>                                                        
    ...
    - [ ] Update `app/profile/page.tsx` to use the plan-based resource limits:   
      ...
      const usageData = [                                                        
        ...
        {                                                                        
          ...
          limit: getResourceLimit(currentPlan?.planType || 'free',               
'AIInteractions'),                                                               
          unit: ""                                                               
        }                                                                        
      ];                                                                         
        ...
        const handleSubmit = async () => {                                       
          if (user && user.ai_interactions_count >= maxInteractions) {           
            toast({                                                              
              ...
              description: `You've reached your plan's limit of                  
${maxInteractions} AI interactions. Please upgrade to continue.`,                
              variant: "destructive",                                            
            });                                                                  
            return;                                                              
          }                                                                      
          ...
          <Card>                                                                 
            ...
            <CardFooter className="flex justify-between">                        
              <p className="text-sm text-muted-foreground">                      
                {user ? `${user.ai_interactions_count || 0}/${maxInteractions}   
interactions used` : ''}                                                         
              </p>                                                               
  ...
  - [ ] Add visual indicators for premium features:                              
    - [ ] Create a `components/ui/premium-badge.tsx` component:                  
      ...
      export function PremiumBadge({ type = 'premium', className = '' }:         
PremiumBadgeProps) {                                                             
        if (type === 'locked') {                                                 
          return (                                                               
...
dark:bg-amber-900/30 dark:text-amber-400 ${className}`}>                         
  ...
  - [ ] Implement graceful degradation for features not available in user's plan:
    - [ ] Create a `hooks/useFeatureAccess.ts` hook:                             
      ...
      export function useFeatureAccess() {                                       
        ...
        const planType = currentPlan?.planType || user?.subscription_plan ||     
'free';                                                                          
...
=> {                                                                             
        ...
        const isNearLimit = (resourceName: string, currentUsage: number,         
threshold = 0.8): boolean => {                                                   
          const limit = getLimit(resourceName);                                  
        ...
        return (                                                                 
          <div className="rounded-lg border p-4 space-y-3">                      
            <div className="flex justify-between items-center">                  
              ...
              <span className={`text-sm ${isAtLimit ? 'text-red-500' :           
isNearLimit ? 'text-amber-500' : ''}`}>                                          
            ...
            <div className="h-2 w-full bg-muted rounded-full overflow-hidden">   
              <div                                                               
                className={`h-full ${isAtLimit ? 'bg-red-500' : isNearLimit ?    
'bg-amber-500' : 'bg-primary'}`}                                                 
      ...
      ```                                                                        
    - [ ] Use this component throughout the application to show limits and       
upgrade prompts                                                                  
